...
namespace Alver.UI.Accounts
{
    public partial class frmClients : Form
    {
        dbEntities db;
        Account _client;
        public frmClients(Account Client)
        {
            InitializeComponent();
        ...
        {
            foreach (Account _account in accounts_InfoBindingSource.List)
            {
                try
                {
        ...
        private void load()
        {
            ...
            accounts_InfoBindingSource.DataSource = _clientsList;
            accounts_FundBindingSource.DataSource = accounts_InfoBindingSource;
        ...
        private void accounts_InfoDataGridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                Account _temp = (accounts_InfoBindingSource.Current as Account);
        ...
        private void accounts_InfoBindingNavigatorSaveItem_Click_1(object sender, EventArgs e)
        {
        ...
        private void Search(string _keyword)
        {
            ...
            {
                accounts_InfoBindingSource.DataSource = db.Accounts.ToList();
            }
            else
            {
                accounts_InfoBindingSource.DataSource = db.Accounts.Where(
                    x =>
        ...
        private void toolStripButton7_Click(object sender, EventArgs e)
        {
            ...
            acc = accounts_InfoBindingSource.Current as Account;
            if (acc != null)
            {
        ...
        {
            accounts_InfoBindingSource.DataSource = null;
        ...
        private void deactivatebtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (accounts_InfoBindingSource.Count <= 0)
                    return;
                    ...
                    if (_client != null)
                    {
                        ...
                        if (_client.Deactivated.Value == true)
                        {
                            ...
                            if (_dialog2 == DialogResult.Yes)
                            {
                                ...
                                {
                                    accounts_InfoBindingSource.EndEdit();
                                    accountsdgv.EndEdit();
        ...
        private void DeleteClient()
        {
            ...
            if (_ConfirmationDialog == DialogResult.Yes)
            {
                ...
                if (_PasswordConfirmationDialog == DialogResult.OK)
                {
                    ...
                    {
                        Account _client = accounts_InfoBindingSource.Current as Account;
                        if (_client != null && accounts_InfoBindingSource.Count > 0)
                        {
                            int _clientId = _client.Id;
                            using (dbEntities db = new dbEntities())
                            {
                                ...
                                EntityState _state = db.Entry(_client).State;
                                if (!db.Accounts.Any(x => x.Id == _clientId))
                                {
                                    accounts_InfoBindingSource.RemoveCurrent();
                                }
